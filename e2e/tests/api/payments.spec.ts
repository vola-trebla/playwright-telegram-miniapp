...
test.describe('Payments / top-up @api @money', () => {
  test('invoice carries the right amount, provider and intent link', async ({ market }) => {
    const invoice = await market.createInvoice(750, 'stars');
  ...
  test('paying an invoice credits the balance', async ({ market }) => {
    ...
    const invoice = await market.createInvoice(1000, 'ton');
    const { balanceTon } = await market.payInvoice(invoice.invoiceId);
    expect(balanceTon).toBe(before + 1000);
  });

  test('paying the same invoice twice is rejected with 409', async ({ market }) => {
    const invoice = await market.createInvoice(200, 'stars');
    await market.payInvoice(invoice.invoiceId);
    const res = await market.payInvoiceResponse(invoice.invoiceId);
    expect(res.status()).toBe(409);
  });

  test('paying an unknown invoice returns 404 with a contract-shaped error', async ({ market }) => {
    const res = await market.payInvoiceResponse('no-such-invoice');
  ...
  test('invalid invoice params are rejected with 400', async ({ market }) => {
    expect((await market.createInvoiceResponse(0, 'stars')).status()).toBe(400);
    expect((await market.createInvoiceResponse(100, 'card')).status()).toBe(400);
  });
  ...
  test('end-to-end: drain, top up via invoice, then buy off the credited balance', async ({
    ...
    await market.withdraw(balanceTon); // → 0
    const invoice = await market.createInvoice(5000, 'ton');
    await market.payInvoice(invoice.invoiceId); // balance → 5000
    const { gift, balanceAfter } = await market.buy('17'); // Retro Boombox, 110 TON
